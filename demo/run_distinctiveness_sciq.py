"""Reproducibility check: does the distinctiveness -> precision-decay finding
(FINDINGS section 7, on OpenBookQA x AG News) reproduce on a SECOND benchmark
with a different answer profile? Here: SciQ (distinctive science-term answers)
x AG News. Same protocol as run_distinctiveness_study.py.
"""
...
CSV_PATH = HERE / "distinctiveness_sciq_results.csv"
...
def make_figure(prec, f1, means):
    ...
    fig.suptitle("SciQ x AG News: answer-only detectability by distinctiveness", y=1.02)
    fig.tight_layout(); fig.savefig(FIG_DIR/"distinctiveness_sciq.png", dpi=130, bbox_inches="tight"); plt.close(fig)

def main():
    print("="*72); print("DISTINCTIVENESS REPRODUCIBILITY  SciQ x AG News"); print("="*72)
    bench = load_jsonl_bench("sciq.jsonl"); corpus = load_corpus()
    dist = answer_distinctiveness(bench, corpus); buckets = terciles(bench, dist)
    print(f"benchmark {len(bench)} SciQ items | corpus {len(corpus)} | terciles {[(b[0],len(b[1]),round(b[2],2)) for b in buckets]}\n")
    prec, f1, means, rows = {}, {}, {}, []
    ...
    print(f"\nCSV -> {CSV_PATH}\nfigure -> {FIG_DIR}/distinctiveness_sciq.png")
    return 0
